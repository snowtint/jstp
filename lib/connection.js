...
class Connection extends EventEmitter {
  ...
  emitRemoteEvent(interfaceName, eventName, args) {
    ...
    const preparedMessage = this._prepareMessage(message);
    this._send(preparedMessage, message);
    this.session._bufferMessage(message.event[0], preparedMessage);
  }
  ...
  handshake(app, login, password, callback) {
    ...
    if (login) {
      if (!password) {
        this.session = login;
        handshakePayload[0] = 'session';
        handshakePayload[1] = [this.session.id, this.session.receivedCount];
        isNewSession = false;
      } else {
        handshakePayload[0] = 'login';
        handshakePayload[1] = [login, password];
      }
    }

    if (version) {
      message = this._createMessageWithArray(
        'handshake', [name, version], ...handshakePayload
      );
    } else {
      message = this._createMessage('handshake', name, ...handshakePayload);
    }
    ...
    if (isNewSession) {
      ...
      this._callbacks[messageId] = (error, receivedCount) => {
        if (!error) {
          this.session._restoreSession(this, receivedCount);
  ...
  _onSessionRestored(error, username, session) {
    ...
    const message = this._createMessage(
      'handshake', null, 'ok', session.recievedCount
    );
    this._send(this._prepareMessage(message), message);
    this.session._resendBufferedMessages();
  ...
  _processCallbackMessage(message) {
    ...
    this.session._onCallbackMessageRecieved(messageId);
  ...
  _processEventMessage(message, keys) {
    ...
    this.session._onMessageRecieved(message.event[0]);
